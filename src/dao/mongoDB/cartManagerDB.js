...
class CartsManager {
    ...
    async addProductToCart(cid, pid) {
        ...
        if (existingProductIndex !== -1) {
            const updateResult = await cartsModel.updateOne({ _id: cid, "products.pid": pid }, { $inc: { "products.$.quantity": 1 } });
            console.log("Cantidad del producto actualizado en el carrito correctamente.");
            return updateResult;
        } else {
            const updateResult = await cartsModel.updateOne({ _id: cid }, { $push: { products: { pid: pid, quantity: 1 } } });
